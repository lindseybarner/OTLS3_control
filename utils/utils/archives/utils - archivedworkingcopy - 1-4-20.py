...
class writeBDV_vis(object): 

	def __init__(self, f, img_3d, idx, binFactor): 

		self.f = f
		self.img_3d = img_3d #shape should be (X(scan direction), Z(camera vertical), Y(horizontal))
		self.idx = idx
	...
	def run(self):
		...
		for z in range(len(res_list)-1, -1, -1):
			...
			subdiv_np[z, 1] = chunkSize2 #chunk size for Z (vertical direction)
			subdiv_np[z, 2] = chunkSize3 #chunk size for Y
			...
			#data = self.f.require_dataset('/t00000/s' + str(self.idx).zfill(2) + '/' + str(z) + '/cells', chunks = (chunkSize1, chunkSize2, chunkSize3), dtype = 'int16', shape = img_3d_temp.shape, compression = "gzip")			
			data[:] = img_3d_temp
	...
	def run(self):
		...
		for z in range(len(res_list)-1, -1, -1): #if res_list = [1,2,4,8], then Z = 3,2,1,0
			...
			if z != 0:
				...
				#LINDSEY MANUALLY COMMENTED LINE BELOW AND UNCOMMENTED THE LINE BELOW LINE BELOW
				...
				#print('chunk sizes= ' + str(chunkSize1) + str(chunkSize2) + str(chunkSize3))
				#print('data sizes = ' + str(img_3d_temp.shape))
				#data = self.f.require_dataset('/t00000/s' + str(self.idx).zfill(2) + '/' + str(z) + '/cells', chunks = (chunkSize1, chunkSize2, chunkSize3), dtype = 'int16', shape = img_3d_temp.shape)			
				data[:] = img_3d_temp
...
def write_xml(drive, save_dir, idx, idx_tile, idx_channel, channels = 1, tiles_y = 1, tiles_z = 1, sampling = 0.2, binning = 1, offset_y = 370, offset_z = 35, x = 1, y = 1, z = 1):
...
def write_xml_vis(drive, save_dir, idx, idx_tile, idx_channel, channels = 1, tiles_y = 1, tiles_z = 1, sampling = .20, binning = 1, offset_y = 370, offset_z = 35, x = 1, y = 1, z = 1):
	
	print("Writing BigDataViewer XML file...")

	c = channels
	tx = tiles_y #number of tiles in y (horizontal?) direction
	tz = tiles_z
	t = tx*tz
	sx = sampling
	binFactor = binning
	...
	f = open(drive + ':\\' + save_dir + '\\vis' + str(idx) + '.xml', 'w')
	f.write('<?xml version="1.0" encoding="UTF-8"?>\n')
	f.write('<SpimData version="0.2">\n')
	f.write('\t<BasePath type="relative">.</BasePath>\n')
	f.write('\t<SequenceDescription>\n')
	f.write('\t\t<ImageLoader format="bdv.hdf5">\n')
	f.write('\t\t\t<hdf5 type="relative">vis' + str(idx) + '.h5</hdf5>\n')
	f.write('\t\t</ImageLoader>\n')
	f.write('\t\t<ViewSetups>\n')
		...
		for j in range(0, tz):
			for k in range(0, tx):
				...
				if ind <= idx:
					...
					shiftx = (ox/sx)*k #LINDSEY NEGATED TO BE CONSISTENT W LINE 263 ...test, undid negative 11/25/19
					shifty = (y/math.sqrt(2.0)-(oz/sz))*j #LINDSEY NEGATIED TO BE CONSISTENT WITH LINE 264 ...test, undid negative 11/25/19
