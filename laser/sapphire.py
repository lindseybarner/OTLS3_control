...
class SapphireLaser(devices.SerialDeviceMixIn, devices.LaserDevice):
    ...
    def __init__(self, com=None, baud=19200, timeout=.5, **kwargs):
        ...
        # headID = self.send(b'?hid')
        # print(isinstance(headID, bytes))
        # print('tried to connect')
        # print(self.connection)
        # while isinstance(headID, bytes) == True:
        #     print('enter loop')
        #     self.connection.close()
        #     print(self.connection.is_open)
        #     print(self.connection)
        #     super().__init__(**kwargs)
        #     self.connection = serial.Serial(port = com, baudrate = baud, timeout = timeout, stopbits = serial.STOPBITS_ONE,bytesize = serial.EIGHTBITS, parity = serial.PARITY_NONE)
        #     print(self.connection)
    ...
    @devices.SerialDeviceMixIn.lock_comms
    def turnOn(self): #_on_enable(self):
        ...
        self.connection.timeout = max(2, prevTimeout)
        isON = self.get_is_on()
    ...
    ## Turn the laser OFF.
    @devices.SerialDeviceMixIn.lock_comms
    def turnOff(self): #disable(self):
        _logger.info("Turning laser OFF.")
        return self._write(b'l=0')

    ## Turn the laser OFF.
    @devices.SerialDeviceMixIn.lock_comms
    def shutDown(self): #disable(self):
        # Disable laser.
        self._write(b'l=0')
        self.flush_buffer()
    ...
    @devices.SerialDeviceMixIn.lock_comms
    def setPower(self, mW): #_set_power_mw(self, mW):
        mW_str = '%.3f' % mW
